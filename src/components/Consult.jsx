...
export default function Consult() {
  ...
  const handleSubmit = () => {
    ...
    const text = [
      "Hello Fortutech! I'd like to book a consultation.", "",
      `*Name:* ${formData.name}`,
    ...
    <section id="consult" className="relative overflow-hidden py-24 px-[5%] bg-off">
      ...
      <div className="relative z-10 grid lg:grid-cols-2 gap-16 items-start max-w-6xl mx-auto">
        ...
        <div ref={leftRef} className="slide-left">
          <p className="font-inter text-[0.68rem] font-bold tracking-[0.2em] uppercase purple mb-3">
            Get In Touch
          </p>
          <h2 ref={headingRef} className="fade-up font-bebas text-4xl lg:text-5xl purple leading-none mb-5">
            Book A Free <span className="amber">QOUTE</span>
          </h2>
          <p className="font-inter text-sm gray-2 leading-relaxed mb-8 max-w-md">
          ...
          <div className="flex flex-col gap-3">
            {contacts.map(({ icon, label, value }, i) => (
              <div
                ...
                className="contact-anim flex items-center gap-4 p-4 border border-[#f5a623]/10 bg-white hover:border-[#f5a623]/40 transition-colors"
              ...
              >
                ...
                <div>
                  <p className="font-inter text-[0.68rem] uppercase tracking-wider gray-2">{label}</p>
                  <p className="font-inter text-sm font-bold purple mt-0.5">{value}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* RIGHT — FORM */}
        <div ref={formRef} className="slide-right bg-white border border-[#f5a623]/15 shadow-[0_8px_32px_rgba(0,0,0,0.06)] p-10">
          <h3 className="font-bebas text-2xl purple mb-1">Request A Quote</h3>
          <p className="font-inter text-sm gray-2 mb-6">We'll reach out on WhatsApp within minutes.</p>
          ...
          <button
            ...
            className={`w-full mt-4 flex items-center justify-center gap-2 py-3.5 text-sm font-bold uppercase tracking-wider transition-colors ${
              cooldown > 0 ? "bg-opacity-50 text-white cursor-not-allowed" : "bg-amber text-white "
            }`}
          >
